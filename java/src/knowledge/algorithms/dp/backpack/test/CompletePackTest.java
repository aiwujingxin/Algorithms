...
import knowledge.algorithms.dp.backpack.complete.*;
...
public class CompletePackTest {
    ...
    public static void testKnapsack() {
        // 创建背包问题测试器
        UniversalTester<CompletePack> tester = new UniversalTester<>(
                CompletePack.class,
                "backPack",
                inputs -> {
                    int[] C = (int[]) inputs[0];
        ...
        // 自动注册所有实现
        CompletePackTest.SolverAutoRegister.autoRegister(tester, CompletePack.class);

        // 对拍测试
        System.out.println("开始背包问题对拍测试...");
        randomKnapsackTest(tester, 100);
        System.out.println("对拍测试完成！");
    }

    public static void randomKnapsackTest(UniversalTester<CompletePack> tester, int testCases) {
        ...
        for (int i = 1; i <= testCases; i++) {
            ...
            // 使用可靠的解法作为基准
            int expected = new Complete_backtrack().backPack(C, W, V);
            // 测试所有解法
            Object[] inputs = {C, W, V};
    ...
    static class SolverAutoRegister {
        ...
        public static <T> void autoRegister(UniversalTester<T> tester, Class<T> interfaceClass) {
            ...
            // 手动列出所有实现类
            Class<?>[] knownClasses = {
                    Complete_dfs_memo.class,
                    Complete_dp_1d.class,
                    Complete_dp_2d.class,
                    Complete_backtrack.class,
            };
