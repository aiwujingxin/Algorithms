...
public class LeetCode140_dfs {
    ...
    private void backtrack(String s, StringBuilder sb, TrieNode root, Set<String> res, int index) {
        if (index >= s.length() && root.isEnd) {
            res.add(sb.toString());
        } else if (index < s.length()) {
            TrieNode next = root.childs[s.charAt(index) - 'a'];
            if (next != null) {
                ...
                if (next.isEnd && !res.contains(tempSb.toString())) {
                    tempSb.append(" ");
                    backtrack(s, tempSb, this.root, res, index + 1);
                }
            }
        }
    }
